...
async def chat_node(state: AgentState, config: RunnableConfig) -> Command[Literal["tool_node", "__end__"]]:
    ...
    print(f"DEBUG: chat_node called with state: {state}")
    print(f"DEBUG: messages in state: {state.get('messages', [])}")
    ...
    # Handle CopilotKit actions if available, otherwise use empty list
    copilotkit_actions = []
    if "copilotkit" in state and "actions" in state["copilotkit"]:
        copilotkit_actions = state["copilotkit"]["actions"]
    ...
    print(f"DEBUG: About to call model with messages: {[system_message, *state['messages']]}")
    ...
    if isinstance(response, AIMessage) and response.tool_calls:
        # Get CopilotKit actions if available, otherwise use empty list
        actions = []
        if "copilotkit" in state and "actions" in state["copilotkit"]:
...
# Always compile without checkpointer for FastAPI integration
